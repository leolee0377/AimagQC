function [FM,BM,SM,CM,TM] = QC_imageMeasurements_TCT_2(BLOCKMASK,block)
...
TEST = imclose(I,strel('disk',1)) -imopen(I,strel('disk',1)) ;
% (100.*sum(sum((bwareaopen(TEST>20,5)).*BLOCKMASK))/sum(BLOCKMASK(:))) ;
% 
% Iorg = double(block) ;
...
entroMASK =  J>mean2(J(J>0)) ;
% add up areas that are flat ( texture wise) but still higher intensity
Me = imbinarize(I.*~entroMASK,quantile(I(I>0),0.5)) ;
% cropp the mask:
...
FM = FMI ;
FakeFocus_Trheshold = 3; 


if (100.*sum(sum((bwareaopen(TEST>20,20)).*BLOCKMASK))/sum(BLOCKMASK(:)))<2
...
Minthresh_25 = quantile(I(I>0),0.75) ;
...
% 
% subplot(2,3,5)
% imagesc(TEST) ; title('close - open 1')
% axis equal
% axis tight
% axis off
% 
