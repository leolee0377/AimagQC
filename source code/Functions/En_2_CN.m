...
for i = 1:numel(allTitle)
    ...
    if (~isempty(allTitle(i).Title) && (strcmp(allTitle((i)).Type,'axes'))==0)
        IDX =  find(strcmp(allTitle(i).Title,T.T1)) ;
        if ~isempty(IDX)
            allTitle(i).Title = T.T2{IDX} ;
        end
    end
end
...
for i = 1:numel(allText)
    if ~isempty(allText(i).Text)
        IDX = find(strcmp(allText(i).Text,T.T1)) ;
        if ~isempty(IDX)
            allText(i).Text = T.T2{IDX} ;
        end
    end
end
...
for i = 1:numel(allString)
    if ~isempty(allString(i).String)
        IDX = find(strcmp(allString(i).String,T.T1)) ;
        if ~isempty(IDX)
            allString(i).String = T.T2{IDX} ;
        end
    end
end
